...
const STATI = ["imballato","spedito","arrivato","pronte per la vendita","venduta"] as const;
type Stato = typeof STATI[number];

type Scatola = {
  id: string; codice: string; colore: string; tipoOggetto: string; luogoProduzione: string;
  progressivo: number; stato: Stato; prezzoBase?: 10|20|30; donazioneLibera?: number;
  dataVendita?: string; note?: string; createdAt: string; updatedAt: string;
};
...
function fmtEUR(n?: number){ if(n==null||isNaN(n)) return ""; return n.toLocaleString("it-IT",{minimumFractionDigits:2,maximumFractionDigits:2}); }
function todayIT(){ return new Date().toLocaleDateString("it-IT"); }
function uuid(){ return Math.random().toString(36).slice(2)+Math.random().toString(36).slice(2); }

const LS_KEY="anffas-scatole-v1";
function loadScatole(): Scatola[]{ try{ const raw=localStorage.getItem(LS_KEY); if(!raw) return []; const arr=JSON.parse(raw); return Array.isArray(arr)?arr:[]; }catch{ return []; } }
function saveScatole(items: Scatola[]){ localStorage.setItem(LS_KEY, JSON.stringify(items)); }
  ...
  return (
    <div className="bg-white border-b border-gray-200 sticky top-0 z-10">
      <div className="mx-auto max-w-7xl flex items-center gap-4 p-4">
        ...
        <button onClick={onOpenLabels} className="ml-auto rounded-xl border px-3 py-1 bg-anffas-yellow/60 text-anffas-blue">
          Stampa etichette
        </button>
      </div>
    </div>
  ...
  return (
    <div className="rounded-2xl bg-white p-5 shadow border border-gray-200">
      <div className="flex items-center justify-between">
        ...
        <button onClick={()=>setOpen(!open)} className="rounded-xl border px-3 py-1">{open?"Nascondi":"Mostra"}</button>
      </div>
      {open && (
        <ol className="mt-3 list-decimal pl-5 space-y-1 text-sm">
          <li><b>HUB</b>: Tipo (3 lett), Luogo (3 lett), Progressivo → genera <i>codice parlante</i>.</li>
          <li>Colore/Note → <b>Aggiungi</b> (stato: <b>imballato</b>).</li>
          <li>Ritiro corriere → stato <b>spedito</b>.</li>
          <li>Filiale: <b>arrivato</b> → <b>pronte per la vendita</b>.</li>
          <li>Vendita: <b>10/20/30</b> + <b>donazione libera</b> + <b>data</b> → stato <b>venduta</b>.</li>
          <li>Totali aggiornati e <b>Esporta CSV</b> nei filtri.</li>
        </ol>
      )}
    </div>
  );
...
function Labels({ items, onBack }: { items: Scatola[]; onBack: () => void }){
  ...
  useEffect(()=>{ (async()=>{
    for(const s of items){
      ...
      if(canvas){ await QRCode.toCanvas(canvas, s.codice, { margin:1, width:160 }); }
    }
  })(); },[items]);
    ...
    <div>
      <div className="mb-4 flex items-center justify-between">
        <div className="flex flex-wrap items-center gap-2">
          ...
          <label className="ml-2 text-sm">Etichette per riga:
        ...
        <div className="text-sm text-gray-600">Consiglio: Carta adesiva A4 – QR 160px.</div>
      </div>
      ...
      <div className="grid gap-4" style={{ gridTemplateColumns:`repeat(${perRow}, minmax(0, 1fr))` }}>
        {items.map((s)=>(
          <div key={s.id} className="rounded-xl border p-3 bg-white break-inside-avoid">
            ...
            <div className="mt-1 text-xs text-gray-600">Tipo: <b>{s.tipoOggetto}</b> • Luogo: <b>{s.luogoProduzione}</b> • Prog: <b>{pad3(s.progressivo)}</b></div>
...
export default function App(){
  ...
  const [query,setQuery]=useState(""); const [filtroStato,setFiltroStato]=useState<string>("");
  ...
  const countsPerStato=useMemo(()=>{ const m=new Map<Stato,number>(); STATI.forEach(st=>m.set(st,0));
    scatole.forEach(s=>m.set(s.stato,(m.get(s.stato)||0)+1)); return m; },[scatole]);

  const visibili=useMemo(()=>{ const q=query.trim().toLowerCase();
    return scatole.filter(s=>{
      const matchQ=!q||s.codice.toLowerCase().includes(q)||(s.colore||"").toLowerCase().includes(q)||
                   (s.note||"").toLowerCase().includes(q)||s.tipoOggetto.toLowerCase().includes(q)||s.luogoProduzione.toLowerCase().includes(q);
      const matchStato=!filtroStato||s.stato===filtroStato; return matchQ&&matchStato;
    });
          ...
          <div className="mx-auto max-w-7xl space-y-6">
            ...
            <Card>
              ...
              <div className="grid gap-3 md:grid-cols-3">
                <input className="rounded-2xl border p-3" placeholder="Cerca per codice, colore, note, tipo o luogo…" value={query} onChange={(e)=>setQuery(e.target.value)} />
...
function Header({ countsPerStato, totaleBase, totaleDonazioni }:{
  countsPerStato: Map<Stato,number>; totaleBase:number; totaleDonazioni:number;
}){
    ...
    <div className="grid gap-4 md:grid-cols-3">
      ...
      <div className="rounded-2xl bg-white p-5 shadow border-t-4 border-anffas-light">
        ...
        <ul className="mt-2 grid grid-cols-2 gap-1 text-sm">
          {STATI.map(s=>(
            <li key={s} className="flex justify-between"><span>{s}</span><span className="font-semibold">{countsPerStato.get(s)||0}</span></li>
          ))}
        </ul>
      </div>
...
function Card({children}:{children:React.ReactNode}){ return <div className="rounded-2xl bg-white p-6 shadow">{children}</div>; }
function CardTitle({children}:{children:React.ReactNode}){ return <h2 className="mb-4 text-2xl font-bold text-anffas-blue">{children}</h2>; }

function NuovaScatolaForm({ onCreate, scatole }:{ onCreate:(s:Scatola)=>void; scatole:Scatola[]; }){
  const [tipo,setTipo]=useState(""); const [luogo,setLuogo]=useState(""); const [progressivo,setProgressivo]=useState<number>(nextProgressivo(scatole));
  const [colore,setColore]=useState(""); const [note,setNote]=useState("");
  ...
  function handleAdd(){
    ...
    const s:Scatola={ id:uuid(), codice, colore:colore.trim(), tipoOggetto:(tipo||"").toUpperCase().slice(0,3),
      luogoProduzione:(luogo||"").toUpperCase().slice(0,3), progressivo, stato:"imballato", note:note.trim()||undefined, createdAt:now, updatedAt:now };
    onCreate(s); setTipo(""); setLuogo(""); setColore(""); setNote(""); setProgressivo(nextProgressivo([...scatole,s]));
  }
...
function nextProgressivo(scatole: Scatola[]){ return Math.min(999, scatole.reduce((acc,s)=>Math.max(acc,s.progressivo),0)+1); }

function ElencoScatole({ items, onUpdate, onDelete }:{
  items:Scatola[]; onUpdate:(id:string, patch:Partial<Scatola>)=>void; onDelete:(id:string)=>void;
}){
  ...
  return <div className="grid gap-4">{items.map(s=><RigaScatola key={s.id} s={s} onUpdate={onUpdate} onDelete={onDelete} />)}</div>;
}
...
function RigaScatola({ s, onUpdate, onDelete }:{
  s:Scatola; onUpdate:(id:string, patch:Partial<Scatola>)=>void; onDelete:(id:string)=>void;
}){
  const [editingVendita,setEditingVendita]=useState(false);
  function advanceStato(){ const idx=STATI.indexOf(s.stato); if(idx<STATI.length-1) onUpdate(s.id,{stato:STATI[idx+1]}); }
      ...
      <div className="mt-4 border-t pt-4">
        ...
        {editingVendita && (
          <FormVendita
            ...
            onSave={(res)=>{
              onUpdate(s.id,{ stato:"venduta", prezzoBase:res.prezzoBase as 10|20|30, donazioneLibera:res.donazioneLibera, dataVendita:res.dataISO });
        ...
        {s.stato==="venduta" && (
          <div className="mt-3 rounded-2xl bg-emerald-50 p-3">
            <div className="text-sm">Prezzo base: <b>€ {fmtEUR(s.prezzoBase)}</b> — Donazione libera: <b>€ {fmtEUR(s.donazioneLibera)}</b></div>
            <div className="text-sm">Totale: <b>€ {fmtEUR((s.prezzoBase||0)+(s.donazioneLibera||0))}</b> — Data vendita: <b>{new Date(s.dataVendita||new Date().toISOString()).toLocaleDateString("it-IT")}</b></div>
          </div>
        )}
      </div>
    </div>
...
function FormVendita({ onSave, onCancel }:{ onSave:(v:{prezzoBase:number; donazioneLibera:number; dataISO:string})=>void; onCancel:()=>void; }){
  ...
  function handleSave(){ onSave({ prezzoBase, donazioneLibera:Number(donazioneLibera||0), dataISO:new Date(data+"T12:00:00").toISOString() }); }
  return (
    <div className="mt-3 grid gap-3 md:grid-cols-5">
      ...
      <div className="flex items-end gap-2 md:col-span-2">
        ...
        <button className="rounded-2xl border px-4 py-2" onClick={onCancel}>Annulla</button>
      </div>
    </div>
  );
}
...
function exportCSV(rows: Scatola[]){
  ...
  const headers=["codice","colore","stato","tipoOggetto","luogoProduzione","progressivo","prezzoBase","donazioneLibera","totaleVendita","dataVendita","note","createdAt","updatedAt"];
  const lines=[headers.join(",")];
  for(const s of rows){
    const row=[ s.codice, s.colore||"", s.stato, s.tipoOggetto, s.luogoProduzione, pad3(s.progressivo),
      s.prezzoBase??"", s.donazioneLibera??"", (s.prezzoBase||0)+(s.donazioneLibera||0),
      s.dataVendita?new Date(s.dataVendita).toLocaleDateString("it-IT"):"", s.note||"",
      new Date(s.createdAt).toLocaleString("it-IT"), new Date(s.updatedAt).toLocaleString("it-IT")
    ].map(v=>`${v}`.replaceAll(",",";"));
    lines.push(row.join(","));
  }
  const blob=new Blob([lines.join("\n")],{type:"text/csv;charset=utf-8;"}); const url=URL.createObjectURL(blob);
  const a=document.createElement("a"); a.href=url; a.download=`anffas_scatole_${new Date().toISOString().slice(0,10)}.csv`; a.click(); URL.revokeObjectURL(url);
}
